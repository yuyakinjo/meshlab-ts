...
/**
 * Nearest point on a mesh's surface, within a distance bound.
 *
 * Upstream walks a uniform grid of faces. This indexes the *barycentres* with
 * the kd-tree already in the library and then does an exact point-to-triangle
 * test on the candidates. It is approximate in one specific way: a very long
 * thin triangle whose barycentre is far from the query can be missed even
 * though part of it is close. On the near-uniform meshes these filters are
 * used with, the candidate pool covers it; the `upperBound` parameter is the
 * honest bound on how far anything is searched at all.
 */
class SurfaceLookup {
